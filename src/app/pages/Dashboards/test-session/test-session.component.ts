...
import { TestSessionService, StartTestResponseDto, TestResultDto, QuestionDto } from '../../../services/test-session.service';
...
export class TestSessionComponent implements OnInit, OnDestroy {
  ...
  startTest(): void {
    ...
    this.testSessionService.startTest().subscribe({
      ...
      error: (err: any) => {
        this.error = err.status === 401 ? 'Foydalanuvchi autentifikatsiyadan o‘tmagan' : 'Testni boshlashda xatolik yuz berdi';
        this.loading = false;
      }
    });
  }
  ...
  submitAnswer(): void {
    if (!this.session || !this.currentQuestion || !this.selectedAnswerId) return;

    this.loading = true;
    this.error = null;

    this.testSessionService.submitAnswer(this.session.sessionId, {
      ...
      next: () => {
        ...
        if (this.session && this.currentQuestionIndex === this.session.questions.length - 1) {
          this.finishTest();
        } else {
          this.currentQuestionIndex++;
          this.currentQuestion = this.session?.questions[this.currentQuestionIndex] || null;
        }
      },
      error: (err: any) => {
        this.error = err.message === 'Vaqt tugagan, test avtomatik tugatildi' ? 
