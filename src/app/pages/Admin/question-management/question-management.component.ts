...
export class ManagementQuestionComponent implements OnInit {
    ...
    addAnswer(answer?: Answer): void {
        if (this.answers.length < 6) {
            ...
            this.answerImagePreviews.push(answer?.imageUrl ? this.sanitizer.bypassSecurityTrustUrl(answer.imageUrl) : null);
        } else {
    ...
    openImage(image: string | SafeUrl | null): void {
        if (image) {
            this.selectedImage = typeof image === 'string' ? this.sanitizer.bypassSecurityTrustUrl(image) : image;
        }
    }
    ...
    private getFormValidationErrors(): string {
        ...
        if (this.answers.controls.some(control => control.errors?.['textOrImage'])) {
