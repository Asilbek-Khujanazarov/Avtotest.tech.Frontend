...
export class TestSessionService {
  ...
  startTest(): Observable<StartTestResponseDto> {
    return this.http.post<StartTestResponseDto>(`${this.api}/Test/start`, {}).pipe(
      catchError(err => throwError(() => new Error(err.error?.message || 'Server error')))
    );
  }

  submitAnswer(sessionId: string, dto: AnswerInputDto): Observable<void> {
    return this.http.post<void>(`${this.api}/Test/${sessionId}/answer`, dto).pipe(
      catchError(err => throwError(() => new Error(err.error?.message || 'Server error')))
    );
  }

  finishTest(sessionId: string): Observable<TestResultDto> {
    return this.http.post<TestResultDto>(`${this.api}/Test/${sessionId}/finish`, {}).pipe(
      catchError(err => throwError(() => new Error(err.error?.message || 'Server error')))
    );
  }
}